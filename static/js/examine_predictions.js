...
function iWindow (iImg) {
    ...
    function build_a_box(iChar){
        ...
        if(iChar.predict_good)
        {
            if(iChar.predict_bad)
            {
                $charBox = $('<div class="char_box"></div>');
            }
            else
            {
                $charBox = $('<div class="char_box_predict_good"></div>');
            }
        }
        else
        {
            if(iChar.predict_bad)
            {
                $charBox = $('<div class="char_box_predict_bad"></div>');
            }
            else
            {
                $charBox = $('<div class="char_box_invisible"></div>');
            }
        }
        $charBox.selectable({
                autoRefresh: false,
...
function startMe( $ ){
    ...
    var  $pages = $.getJSON('/ajax/get_features').done(iWindow);
}
